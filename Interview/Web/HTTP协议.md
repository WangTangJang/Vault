HTTP（Hypertext Transfer Protocol）是一种用于在网络上传输[[Hypertext|超文本]]（Hypertext）数据的协议。它是一种无状态协议，意味着每个HTTP请求都是相互独立的，服务器在处理请求时不会保存请求之间的状态信息。HTTP是Web应用中最常用的协议之一，用于在客户端和服务器之间传输数据。
