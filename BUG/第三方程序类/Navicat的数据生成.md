## Problem
### Description
当向comments中随机生成的数据的时候，
只能生成214条。
一旦想要生成215条，
就错误，然后回滚。
是因为结构的连接太复杂了吗？
但是214条的时候很丝滑，多一条就会循环一会然后回滚。
...
(  
    ...
    -- 到时候查询的时候,直接去统计那张关联表.    -- 而不是在绑定到这张表里    -- 但是 .....    likes             int,  
    dislikes          int,  
...
[DGR] 1> comments: --Transaction - Begin--
[DGR] 1> comments: Preparing field
[DGR] 1> comments: Start records generation
[DGR] 1> comments: 2006 - Server has gone away
[DGR] 1> comments: Records generated 
[DGR] 1> comments: --Transaction - Rollback--
[DGR] Finished with error
